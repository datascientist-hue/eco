...
st.set_page_config(page_title="Overdue Creditor Report", layout="wide")
...
st.subheader("📋 Overdue Creditor Report")
...
with col_download1:
    ...
    st.download_button("📥 Download Full Report (CSV)", csv, "Overdue_Creditor_Report.csv", "text/csv")
...
with col_download2:
    ...
    try:
        ...
        with pd.ExcelWriter(buffer, engine='openpyxl') as writer:
            export_df.to_excel(writer, index=False, sheet_name='Creditor Report')
        buffer.seek(0)
        st.download_button("📊 Download Report (Excel)", buffer.getvalue(), "Overdue_Creditor_Report.xlsx", "application/vnd.openxmlformats-officedocument.spreadsheetml.sheet")
    except:
